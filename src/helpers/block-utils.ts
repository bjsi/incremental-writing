...
export class BlockUtils extends ObsidianUtilsBase {
	...
	async createBlockRefIfNotExists(lineNumber: number, noteFile: TFile, customBlockRef: string = null): Promise<string>{
		const blockRef = await this.getBlockRefHash(lineNumber, noteFile);
		if (blockRef === null){
			return null;
		}
		else if (blockRef !== "") {
			return this.app.metadataCache.fileToLinktext(noteFile, "", true) + "#^" + blockRef;
		}
		else {
	...
	async addBlockRef(lineNumber: number, noteFile: TFile, customBlockRef: string = null): Promise<string> {
		const oldNoteLines = (await this.app.vault.read(noteFile))
			?.split(/\r?\n/) || []
